<div class="d-flex flex-column-reverse">
            ...
            <div class="d-lg-flex align-items-center col-lg-7 mt-1 mt-lg-0">
                ...
                @if($this->searchableColumns()->count())
                    <div class="input-group rounded-lg shadow-sm ">
                        ...
                        <input wire:model.debounce.500ms="search" type="text" class="form-control" aria-label="Search"
                               placeholder="Buscar por {{ strtolower( $this->searchableColumns()->map->label->join(', ') )}}">
                    </div>
                @endif
            </div>
            <!-- / -->
    ...
    <!-- / -->
        ...
        <!-- CONTAINER TABLE -->
        <div
            class="rounded-lg shadow-lg bg-white overflow-x-scroll scrollbar-w-2 scrollbar-track-gray-lighter scrollbar-thumb-rounded scrollbar-thumb-gray     max-w-screen">
            <div
                ...
                <div class="d-table align-middle w-full          min-w-full" >
                    @unless($this->hideHeader)
                        ...
                        @foreach($this->columns as $index => $column)
                            ...
                            @elseif($column['type'] === 'checkbox')
                                <!-- Column Checkbox -->
                                    <div
                                        class="position-relative d-table-cell h-12 w-48 overflow-hidden align-top px-4 py-2 border-bottom border-info text-left font-weight-bold text-black-50 text-uppercase d-flex align-items-center          border-gray-200 bg-gray-50 text-xs leading-4 text-gray-500 tracking-wider flex items-center focus:outline-none">
                                        <div
                                            class="px-3 py-1 rounded @if(count($selected)) bg-orange-400 bg-primary @else bg-secondary @endif text-white text-center">
                                            {{ count($selected) }}
                                        </div>
                                    </div>
                            @else
                        ...
                        <div class="d-table-row divide-x divide-blue-200 bg-blue-100">
                            @foreach($this->columns as $index => $column)
                                ...
                                @elseif($column['type'] === 'checkbox')
                                    <div
                                        class="w-32 overflow-hidden align-top bg-blue-100 px-2 py-3 border-bottom border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 text-uppercase tracking-wider d-flex h-full flex-column align-items-center space-y-2 focus:outline-none">
                    ...
                    @forelse($this->results as $result)
                        <div
                            class="d-table-row p-1 divide-gray-100 {{ isset($result->checkbox_attribute) && in_array($result->checkbox_attribute, $selected) ? 'bg-orange-100' : ($loop->even ? 'bg-gray-100' : 'bg-gray-50') }}">
